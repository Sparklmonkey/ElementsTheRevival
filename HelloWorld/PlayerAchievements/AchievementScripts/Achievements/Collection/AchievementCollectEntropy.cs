...
public class AchievementCollectEntropy : IAchievement
{
    ...
    public int IsBronzeAchieved(SavedData playerData, CardDatabase cardDatabase)
    {
        ...
        return cardDatabase.HasCardSingleCollection(fullCardList, Element.Entropy);
    }

    public int IsGoldAchieved(SavedData playerData, CardDatabase cardDatabase)
    {
        var fullCardList = playerData.GetFullCardList();
        return cardDatabase.HasCardHalfCollection(fullCardList, Element.Entropy);
    }

    public int IsPlatinumAchieved(SavedData playerData, CardDatabase cardDatabase)
    {
        var fullCardList = playerData.GetFullCardList();
        return cardDatabase.HasCardCompleteCollection(fullCardList, Element.Entropy);
    }
}