...
public class CreatureCardDisplay : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler
{
    ...
    {
        if (!updateCardDisplayEvent.Id.Equals(Id))
        {
            return;
        }

        Card = updateCardDisplayEvent.Card;
    ...
    {
        if (!onDisplayCreatureAttackEvent.AttackingId.Equals(Id))
        {
            return;
        }
    ...
    private void HideCard(ClearCardDisplayEvent clearCardDisplayEvent)
    {
        ...
        if (Card.IsAflatoxin)
        {
            ...
            DisplayCard(new UpdateCreatureCardEvent(Id, card));
        } 
    ...
    private void CheckOnPlayEffects()
    {
        ...
        {
            var shardList = DuelManager.Instance.GetIDOwner(Id).playerHand.GetAllCards().FindAll(x => x.card.cardName.Contains("Shard of"));
            Card = CardDatabase.Instance.GetGolemAbility(shardList);
        }
