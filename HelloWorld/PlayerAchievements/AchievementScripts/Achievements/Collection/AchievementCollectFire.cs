...
public delegate int TierCheckDelegate(SavedData playerData, CardDatabase cardDatabase);
public class AchievementCollectFire : IAchievement
{
    ...
    public int IsBronzeAchieved(SavedData playerData, CardDatabase cardDatabase)
    {
        ...
        return cardDatabase.HasCardSingleCollection(fullCardList, Element.Fire);
    }

    public int IsGoldAchieved(SavedData playerData, CardDatabase cardDatabase)
    {
        var fullCardList = playerData.GetFullCardList();
        return cardDatabase.HasCardHalfCollection(fullCardList, Element.Fire);
    }

    public int IsPlatinumAchieved(SavedData playerData, CardDatabase cardDatabase)
    {
        var fullCardList = playerData.GetFullCardList();
        return cardDatabase.HasCardCompleteCollection(fullCardList, Element.Fire);
    }
}